...
parser.add_argument('--model_weights', default=False, action='store_true', help='pretrained model weights')
...
def compute_accuracy(model, testloader):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets, _) in enumerate(testloader):
            ...
            all_correct_preds += correct_preds.item()
            all_preds += inputs.size(0)

    return all_correct_preds * 100.0 / all_preds


def gen_adv(model, x, y):
    x.detach()
    inputs1_adv = Variable(x, requires_grad=True).to(device)
    # generate adversarial example
    tmp_loss = criterion(model(inputs1_adv), y) # CL(net, x, inputs1_adv, gen_adv = True)
    tmp_loss.backward()
...
# log the output
test_log_file = open(log_dir + suffix + '_supervised_full.txt', "w")                
...
if args.adv:  
    net = models.__dict__[args.resnet + '_cifar_supervised'](pool_len = pool_len, low_dim=args.low_dim, bn_adv_flag=True, bn_adv_momentum = args.bn_adv_momentum)
else:
    net = models.__dict__[args.resnet + '_cifar_supervised'](pool_len = pool_len, low_dim=args.low_dim, bn_adv_flag=False)
...
if args.model_weights:
    ...
    print('==> Succesfully loaded weights from: ', model_path)

    # for k, v in net.named_parameters():
    #     if 'clf' in k or 'layer4' in k or 'linear' in k or 'layer3' in k or 'layer2' in k:
...
def train(epoch):
    ...
    for batch_idx, (inputs1, inputs2, targets, indexes) in enumerate(trainloader):
        ...
        inputs2, targets, indexes = inputs2.to(device), targets.to(device), indexes.to(device)
        
        if args.adv:
            inputs1 = inputs1.to(device)
            inputs_adv = gen_adv(net, inputs1, targets)
        
        optimizer.zero_grad()
        loss = criterion(net(inputs2), targets)
        
        if args.adv:
            loss += args.alpha*criterion(net(inputs_adv), targets)
        
        loss.backward()
        optimizer.step()
        train_loss.update(loss.item(), 2*inputs2.size(0))         
...
for epoch in range(start_epoch, start_epoch+301):
    ...
    if epoch%test_epoch ==0:
        ...
        if dataset == 'cifar10' or dataset == 'cifar100':
            acc = compute_accuracy(net, testloader) # kNN(epoch, net, trainloader, testloader, 200, args.batch_t, ndata, low_dim = args.low_dim)
        ...
        if acc >= best_acc:
            ...
            torch.save(state, args.model_dir + '/' + dataset + '/' + suffix + '_supervised_full_best.t')
            best_acc = acc
